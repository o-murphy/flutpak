...
Flutter includes plugins via a symlink (`flutter/ephemeral/.plugin_symlinks/<plugin>/linux`);
the `../objectbox-c` path traverses the symlink transparently and resolves correctly to
the pub-cache package directory at the OS level.
