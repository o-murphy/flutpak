...
extracts the matching `objectbox-c` archive.
...
`objectbox-c` is licensed under Apache 2.0, which may affect Flathub review if the
main app is GPL-3.0. However, `objectbox_flutter_libs` links to `libobjectbox.so`
dynamically at runtime, keeping the two works legally separate (GPLv3 §1). Shipping
them together inside the isolated Flatpak `/app/lib/` qualifies as Mere Aggregation
(GPLv3 §5), which is explicitly permitted.
